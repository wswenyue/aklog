...
class AsciiColor:
    ...
    bg_desc = {
        ...
        "light_green": 102,
        "hl_green": 102,
        "highlight_yellow": 103,
        "light_yellow": 103,
        "hl_yellow": 103,
        "highlight_blue": 104,
        "light_blue": 104,
        "hl_blue": 104,
        "highlight_purple": 105,
        "light_purple": 105,
        "hl_purple": 105,
        "highlight_cyan": 106,
        "light_cyan": 106,
    ...
    def __str__(self):
        if self.style and self.fg and self.bg:
            return "\u001b[" + ";".join([str(self.style), str(self.fg), str(self.bg)]) + "m"
        if self.fg and self.bg:
            return "\u001b[" + ";".join(["0", str(self.fg), str(self.bg)]) + "m"
        if self.fg:
            if self.style:
                return "\u001b[" + f"{self.style};{self.fg}m"
            else:
                return "\u001b[" + f"{self.fg}m"
        if self.bg:
            return "\u001b[" + f"{self.bg}m"
        ...
        if self.style:
            return "\u001b[" + f"{self.style}m"
        return ""

    def format(self, data: str) -> str:
        return str(self) + data + "\u001b[0m"
...
class ColorStr:
    ...
    def set_color(self, begin: int, end: int, color: AsciiColor):
        ...
        self._nodes: List[_ColorNode] = [_node for _node in self._nodes if _node.index < begin or _node.index > end]
        # add beginNode
    ...
    def _format_sub(self, begin_node: _ColorNode, end: int) -> str:
        ...
        if begin_node.color:
            ...
            if not _color:
                raise ValueError("_format_sub color is null!!!")
...
if __name__ == "__main__":
    ...
    print(
        AsciiColor(fg="light_gray").format("11:01:34.321#bang@main#22943❗#")
        + AsciiColor(fg="green", style="underline").format("D")
        + AsciiColor(fg="light_green").format("#ZLogDebug(TraceDataDBMgr)#")
        + str(
            ColorStr("upload msg list is empty!!!", base_color=AsciiColor(fg="green", style="italic"))
